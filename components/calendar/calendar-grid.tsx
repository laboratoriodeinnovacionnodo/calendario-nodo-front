...
const eventTypeBgLight: Record<string, string> = {
  PENDIENTE: 'bg-orange-50 border-l-2 border-l-orange-500 hover:bg-orange-100',
  EN_CURSO: 'bg-blue-50 border-l-2 border-l-blue-500 hover:bg-blue-100',
  FINALIZADO: 'bg-green-50 border-l-2 border-l-green-500 hover:bg-green-100',
  CANCELADO: 'bg-red-50 border-l-2 border-l-red-500 hover:bg-red-100',
  MASIVO: 'bg-purple-50 border-l-2 border-l-purple-500 hover:bg-purple-100',
          ...
          >
            ...
            <div className="space-y-0.5 overflow-hidden">
              {day.events.slice(0, 3).map((event) => {
                const eventStart = event.fechaDesde.split('T')[0]
                const eventEnd = event.fechaHasta.split('T')[0]
                const currentDateStr = `${year}-${String(month).padStart(2, '0')}-${String(day.date).padStart(2, '0')}`
                const isMultiDay = eventStart !== eventEnd
                const isFirstDay = currentDateStr === eventStart
                const showTime = isFirstDay || !isMultiDay
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      isMultiDay && !isFirstDay && 'opacity-90'
                    )}
                    title={`${event.titulo} (${event.horaDesde} - ${event.horaHasta})`}
                  >
                    {showTime && <span className="font-medium">{event.horaDesde}</span>}
                    <span className={cn('ml-1 hidden md:inline', !showTime && 'ml-0')}>{event.titulo}</span>
                    <span className={cn('ml-1 md:hidden', !showTime && 'ml-0')}>{event.titulo.slice(0, 10)}...</span>
                  </Link>
                )
              })}
            </div>
